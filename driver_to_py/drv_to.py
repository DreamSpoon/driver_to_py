...
def create_driver_py_from_data_item(space_pad, make_into_function, animdata_bool_vec):
    ...
    d2p_text = bpy.data.texts.new(D2P_TEXT_NAME)
    d2p_text.write("# Blender Python script to re-create drivers of objects\n")

    if make_into_function:
        d2p_text.write("import bpy\n\n" +
                       "def create_drivers_of_objects():\n")

    text_len_before = len(d2p_text.lines)
    ...
    for c in range(len(ANIMDATA_SOURCES)):
        ...
        if animdata_bool_vec[c]:
            write_py_data_recurse(d2p_text, line_prefix, data_rlookup_table, ANIMDATA_SOURCES[c][1], 0, bpy.data,
                              "bpy.data")
    text_len_after = len(d2p_text.lines)
    # if zero lines were written in function, then write a pass statement
    if text_len_after == text_len_before:
        d2p_text.write(line_prefix + "pass\n")
    ...
    d2p_text.current_line_index = 0
    d2p_text.cursor_set(0)

class DRV2PY_DriversToPython(bpy.types.Operator):
    bl_description = "Convert all drivers of selected data sources to Python code, available in the Text Editor"
    bl_idname = "drv2py.drivers_to_python"
    bl_label = "Drivers to Py"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        scn = context.scene
        create_driver_py_from_data_item(scn.D2P_NumSpacePad, scn.D2P_MakeFunction, scn.D2P_AnimdataBoolVec)
        return {'FINISHED'}
...
class DRV2PY_SelectAllAnimdataSrc(bpy.types.Operator):
    bl_description = "Select all available data sources"
    bl_idname = "drv2py.select_all_animdata_src"
    bl_label = "Select All"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        scn = context.scene
        set_bool_vec_state(scn.D2P_AnimdataBoolVec, True)
        return {'FINISHED'}

class DRV2PY_SelectNoneAnimdataSrc(bpy.types.Operator):
    bl_description = "Select all available data sources"
    bl_idname = "drv2py.select_none_animdata_src"
    bl_label = "Select None"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        scn = context.scene
        set_bool_vec_state(scn.D2P_AnimdataBoolVec, False)
        return {'FINISHED'}
